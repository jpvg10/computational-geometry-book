...
public class Poligonos {
	...
	public static boolean esConvexo(Point2D.Double[] poligono) {
		int n = poligono.length;
		boolean inicial = giro(poligono[0], poligono[1], poligono[2]) >= 0;
		for (int i = 1; i < n; i++) {
			int j = (i + 1) % n;
			int k = (i + 2) % n;
			boolean actual = giro(poligono[i], poligono[j], poligono[k]) >= 0;
			if (actual != inicial)
				return false;
		}
		return true;
	...
	public static boolean puntoEnPoligono(Point2D.Double[] poligono, Point2D.Double p) {
		int n = poligono.length;
		double anguloTotal = 0.0;
		for (int i = 0; i < n; i++) {
			int j = (i + 1) % n;
			if (giro(poligono[i], p, poligono[j]) == 1) {
				anguloTotal += angulo(poligono[i], p, poligono[j]);
			} else {
				anguloTotal -= angulo(poligono[i], p, poligono[j]);
			}
		}
	...
	public static Point2D.Double[] convexHull(Point2D.Double[] puntos) {
		...
		if (k > 1) {
			hull = Arrays.copyOfRange(hull, 0, k - 1); // Quitar vertices que no pertenecen al hull despues de k
		}
